...
The worktree already exists at $worktreePath on branch $branchName, and you are already running in it. Do not create a Git worktree of your own for this work, and do not relocate this one. Other terminal panes are attached to this exact path.
...
Continue through all requested issues unless blocked. Do not push, merge, create additional worktrees, remove the worktree, or modify files in the original checkout. Preserve any pre-existing changes if this is a resumed worktree. At the end, summarize each issue's status and commit.
"@
...
The Codex pane is implementing the work. All three panes are attached to this worktree path, so do not create a Git worktree of your own and do not relocate this one. The PowerShell pane can run parameterless `keclose` from this worktree after we agree the exact HEAD is ready.
"@
