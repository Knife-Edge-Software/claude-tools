...
### Step 6: Signal Batch Orchestrator (if in batch mode)

Check if running in batch mode and signal completion:

```bash
# Check for batch state directory
if [ -d "ke-batch-state" ] || [ -d "../ke-batch-state" ]; then
    # Running in batch mode - signal completion
    /ke:batch-signal complete <issue-number>
fi
```

**Batch mode detection:**
- Look for `ke-batch-state/` directory in current or parent directory
- If found, automatically signal completion after closing the issue
- This allows the orchestrator to track progress and trigger dependent work

**Signal on failure:**
If the close operation fails (push error, merge conflict, etc.), signal failure instead:
```bash
/ke:batch-signal failed <issue-number> --reason "<error description>"
```
...
- **Batch mode:** When `ke-batch-state/` directory exists, automatically signal completion/failure to orchestrator
